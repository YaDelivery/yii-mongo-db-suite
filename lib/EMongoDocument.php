...
abstract class EMongoDocument extends EMongoEmbeddedDocument
{
  ...
  public function setFsyncFlag($flag)
  {
    ...
    if ($this->_fsyncFlag)
    {
      $this->setSafeFlag(true);
    } // Setting FSync flag to true will implicit set safe to true
  }
   ...
   * - Global level (always set)
   *
   * @param EMongoCriteria $criteria query criteria.
   *
   * @return boolean
   */
  public function getUseCursor($criteria = null)
  {
    ...
    {
      return self::$_models[get_class($this)]->useCursor;
    } // Model have flag set, return it
  ...
  {
    Yii::trace(get_class($this) . '.exists()', 'ext.MongoDb.EMongoDocument');
  ...
  private function createPkCriteria($pk, $multiple = false)
  {
    ...
    if (is_string($pkField))
    {
      ...
      if (!$multiple)
      {
        $criteria->{$pkField} = array_shift($pk);
      }
      else
      {
